...
if __name__=="__main__":
    ...
    ## 입력데이터(X)와 정답레이블(y) 분리
    # pca는 데이터의 분산이 가장 큰방향 찾기(라벨사용안함, 비지도학습)
    # lda는 클래스간 구분이 잘되는 방향찾기(라벨사용, 지도학습)
    X_data = df.iloc[:, :-1] # 전체를 가져오되 마지막꺼 cluster_label 제거
    y_data = df.iloc[:, -1].values # 전체를 가져오되 labeling된 마지막껏만 안가져옴

    ## LDA 기법 적용
    # 차원축소
    lda = LDA(n_components=2) # lda도 pca가 기본적으로 들어감
    X = lda.fit_transform(X_data, y_data)
    df['feature1'] = X[:, 0]
    df['feature2'] = X[:, 1]
    ...
    for label in range(3):
        plt.scatter(
            df.loc[df['cluster_label']==label, 'feature1'], #fancyindexing 사용됨
            df.loc[df['cluster_label']==label, 'feature2'],
            color=colors[label],
            # cmap='viridis',
            edgecolors='k',
            s=100
        )
    plt.xlabel("First Linear Discriminant") # 같은 차원 축소지만 라벨을 알고 있는 상태에서 한다는 차이가 있음
